	...
	for (const [name, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			...
			const requestType = caretRequestTypes.includes(rpc.requestType.type.name)
				...
				: `cline.${rpc.requestType.type.name}`
			const responseType = caretResponseTypes.includes(rpc.responseType.type.name)
				? `caret.${rpc.responseType.type.name}`
				: `cline.${rpc.responseType.type.name}`
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
			}
			if (rpc.responseStream) {
...
import { cline, caret } from "@generated/grpc-js"
import { Controller } from "@core/controller"
