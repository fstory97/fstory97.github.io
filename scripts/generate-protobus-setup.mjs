...
async function generateStandaloneProtobusServiceSetup(protobusServices) {
	const imports = []
	const handlerSetup = []
	// CARET MODIFICATION: Track which packages are used
	const usedPackages = new Set()

	for (const [name, def] of Object.entries(protobusServices)) {
		const domain = getDomainName(name)
		const dir = getDirName(name)
		// CARET MODIFICATION: Use the service's package name (defaults to 'cline')
		const packageName = def.packageName || "cline"
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			...
			// CARET MODIFICATION: Use getFqn to get fully qualified type names
			const requestType = getFqn(rpc.requestType.type.name)
			const responseType = getFqn(rpc.responseType.type.name)
			if (rpc.requestStream) {
				throw new Error("Request streaming is not supported")
	...
	// CARET MODIFICATION: Generate package imports based on what's used
	const packageImports = Array.from(usedPackages)
		.sort()
		.map((pkg) => `import { ${pkg} } from "@generated/grpc-js"`)
		.join("\n")
