...
焦点链是 Cline 中的任务管理增强功能，提供自动待办事项列表管理，在整个任务过程中进行实时进度跟踪。
...
这使 Cline 能够处理长期任务，无缝管理发送到 LLM 的上下文，并在多次上下文窗口重置中保持 Cline 的正轨。

<Tip>
  焦点链与 Cline 的[深度规划斜杠命令](/features/slash-commands/deep-planning)配合得特别好，为通过[规划过程](/features/plan-and-act)创建的实施任务提供无缝的进度跟踪。
</Tip>
