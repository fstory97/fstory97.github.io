...
当我收到新的 PR 要审查时，我过去需要手动收集上下文：检查 PR 描述、检查差异、查看周围文件，最后形成意见。现在我只需要：
...
-   检查周围文件以获取上下文
-   分析潜在问题
-   如果一切看起来都很好，询问我是否可以批准它，并给出应该批准的理由
-   如果我说"是"，Cline 会自动使用 `gh` 命令批准 PR

这已经将我的 PR 审查过程从手动的多步骤操作转变为单个命令，为我提供做出明智决策所需的一切。

> 这只是工作流程文件的一个示例。您可以在我们的[提示存储库](https://github.com/cline/prompts)中找到更多灵感。

## 构建您自己的工作流程

工作流程的美妙之处在于它们完全可以根据您的需求进行自定义。您可以为各种重复性任务创建工作流程：

-   对于发布，您可以创建一个工作流程来获取所有合并的 PR、构建更改日志并处理版本升级。
-   设置新项目非常适合工作流程。只需运行一个命令即可创建文件夹结构、安装依赖项并设置配置。
-   需要创建报告？创建一个工作流程，从不同来源获取统计信息，并按您喜欢的方式格式化它们。您甚至可以使用图表库将它们可视化，然后使用 [slidev](https://sli.dev/) 等库制作演示文稿。
-   您甚至可以使用工作流程在提交 PR 后使用 Slack 或 Whatsapp 等 MCP 服务器向您的团队起草消息。

使用工作流程，您的想象力就是极限。真正的潜力来自发现那些您一直在做的烦人的重复性任务。

如果您可以将某事描述为"首先我做 X，然后 Y，然后 Z" - 那就是完美的工作流程候选者。

从一些困扰您的小事开始，将其变成工作流程，并不断完善它。您会惊讶于这种方式可以自动化您一天中的多少内容。
