...
export interface CaretModelInfoResponse {
	data: Array<{
		model_name: string
		litellm_params: {
			model: string
...
export class CaretHandler implements ApiHandler {
	...
	private modelInfoCache: CaretModelInfoResponse | undefined
	...
	private ensureClient(): OpenAI {
		if (!this.client) {
			if (!this.options.caretApiKey) {
				throw new Error("Caret API key is required")
			}
			try {
	...
	private async modelInfo(publicModelName: string): Promise<CaretModelInfoResponse["data"][number] | undefined> {
	...
	private async fetchModelsInfo(): Promise<CaretModelInfoResponse | undefined> {
		...
		try {
			const response = await fetch(url, {
				...
				headers: {
					...
					"x-litellm-api-key": this.options.caretApiKey || "",
				},
			})

			if (response.ok) {
				const data: CaretModelInfoResponse = await response.json()
				this.modelInfoCache = data
				this.modelInfoCacheTimestamp = now
				return data
			} else {
				console.warn("Failed to fetch Caret model info:", response.statusText)
				...
				if (retryResponse.ok) {
					const data: CaretModelInfoResponse = await retryResponse.json()
					this.modelInfoCache = data
					this.modelInfoCacheTimestamp = now
					return data
				} else {
					console.warn("Failed to fetch Caret model info with Authorization header:", retryResponse.statusText)
					return undefined
				}
			}
		} catch (error) {
			console.warn("Error fetching Caret model info:", error)
			return undefined
		}
	}
	...
	private async getModelCostInfo(publicModelName: string): Promise<{
		...
		try {
			...
			console.warn("Error getting Caret model cost info:", error)
		}
	...
	async calculateCost(
		...
		const publicModelId = this.options.caretModelId || caretDefaultModelId
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		const modelId = this.options.caretModelId || caretDefaultModelId
		...
		let temperature: number | undefined = this.options.caretModelInfo?.temperature ?? 0
		...
		const cacheControl =
			this.options.caretUsePromptCache && Boolean(modelInfo?.model_info.supports_prompt_caching)
		...
		const stream = await client.chat.completions.create({
			model: this.options.caretModelId || caretDefaultModelId,
			messages: [systemMessage, ...enhancedMessages],
			temperature,
			stream: true,
			stream_options: { include_usage: true },
			...(thinkingConfig && { thinking: thinkingConfig }), // Add thinking configuration when applicable
			...(this.options.ulid && { litellm_session_id: `caret-${this.options.ulid}` }), // Add session ID for Caret tracking
		})
