...
```typescript
import WebviewLogger from "./CaretWebviewLogger"
...
**주요 기능:**
- 안전한 webview 로깅 (오류로 인한 webview 로딩 실패 방지)
- 개발/프로덕션 모드 자동 감지
- 확장 호스트와의 안전한 통신
- 브라우저 콘솔 로깅 지원
...
실제 구현 위치: `webview-ui/src/caret/utils/urls.ts`

```typescript
import { CARET_URLS, CARET_LOCALIZED_URLS } from "./urls"

// Caret 서비스 URL
const serviceUrl = CARET_URLS.CARET_SERVICE
const githubUrl = CARET_URLS.CARET_GITHUB

// 다국어 지원 URL
const getLocalizedUrl = (key: string, locale: string = "en") => {
	return CARET_LOCALIZED_URLS[locale]?.[key] || CARET_URLS[key]
}
```

**지원 URL 카테고리:**
- Caret 서비스 관련 URL (공식 사이트, 계정, 크레딧)  
- GitHub 저장소 URL (메인, 문서, 이슈)
- 외부 서비스 URL (AI 프로바이더 등)
- 다국어별 특화 URL
...
- 2025-08-29: f01 공통 유틸리티 포트 완료 - CaretWebviewLogger, urls.ts 추가
