...
- **UIの改善とバグ修正**:
  - **多言語サポートの強化**: 中国語と日本語で不足していた翻訳を追加しました。
  - **バグ修正**: ショートカットキーのバグと、コンテキスト付きで新しいタスクを開始する際のエラーを修正しました。
  - **ドキュメントリンクの修正**: ユーザーマニュアルへのリンクを更新しました。
  - **UI調整**: 韓国語環境で「エージェント」ボタンが二行で表示される問題を解決しました。
